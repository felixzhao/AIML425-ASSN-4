...
To resolve the problem we found for basic autoencoder which is latent variables distribution unstable.
We selected Variational autoencoder (VAE) to control the distribution of the latent (bottleneck) variables,
and then add iid Gaussian noise to the latent (bottleneck) variable with an adjustable (not learned) SNR
...
We experimented with different parameter values.
SNR in range 5, 10, 20, 50, 100.
latent dimensionalities in range 2, 3, 5, 7, 10.

Figure $\ref{fig:discuss}$ shows, when latent dimension is low the latent can not represent all information form the input.
The reconstructed oupt appears banded. 
when latent dimension is hight, it shows more infomation.

For SNR, compare with low value, it show more scattered which high value.
\begin{figure}[htb]
  \centering
  % First row
...
For 3D point clouds (like the surface of a cube), the Chamfer distance or EMD are commonly used.
In this work, we choice Earth Mover's Distance (EMD) to evaluate the model performance

Base on the EMD score, the final model select latent dimensionalities as 10, and SNR as 50 in the previous given range.
